...
export async function POST(request: NextRequest) {
  ...
  try {
    if (body.action === 'schedule') {
      ...
      const video = await scheduleYouTubeVideo({ videoId: body.videoId, scheduledAt: body.scheduledAt });
    ...
    if (body.action === 'publish_now') {
      ...
      const video = await publishYouTubeVideo({ videoId: body.videoId, privacyStatus: body.privacyStatus });
